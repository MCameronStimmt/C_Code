...
//ensure user enters valid number or q/Q 
void validDonation(Donation* donPtr, Organization* orgPtr) {
	bool cont = false;
	double intTest = 0;
	//continue while incorrect input
	while (!cont) {
		...
		//send user to organization login if q/Q is entered
		if (strcmp(donPtr->amount, "q") == 0 || strcmp(donPtr->amount, "Q") == 0) {
			cont = true;
			login(donPtr, orgPtr);
		}
		//if not q, then test for integer within range
		else {
			//test for valid integer
			char* end;
			errno = 0;
...
void displayFundraiser(Organization* orgPtr) {
	...
	if (amount >= orgPtr->donations) {
		puts("We have reached our goal but can still use the donations.");
	}
	else {
		printf("We are %f percent towards our goal of %.2f", percent, amount);
	}
}
